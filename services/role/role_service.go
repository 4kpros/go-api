...
type RoleService interface {
	...
	FindById(id string) (role *model.Role, errCode int, err error)
	FindAll(filter *types.Filter, pagination *types.Pagination) (roles []model.Role, errCode int, err error)
}
...
func (service *RoleServiceImpl) Create(role *model.Role) (errCode int, err error) {
	...
	if foundRole != nil && foundRole.Name == role.Name {
		message := "This role already exists! Please enter anther one."
...
func (service *RoleServiceImpl) Delete(id string) (affectedRows int64, errCode int, err error) {
	...
	if affectedRows <= 0 {
		...
		message := "Could not delete role that doesn't exists! Please enter valid id."
		err = fmt.Errorf("%s", message)
		return
	}
	return
...
func (service *RoleServiceImpl) FindById(id string) (role *model.Role, errCode int, err error) {
	...
	if role == nil {
		errCode = http.StatusNotFound
		message := "Role not found! Please enter valid id."
		err = fmt.Errorf("%s", message)
	}
	return
}

func (service *RoleServiceImpl) FindAll(filter *types.Filter, pagination *types.Pagination) (roles []model.Role, errCode int, err error) {
	roles, err = service.Repository.FindAll(filter, pagination)
	if err != nil {
		errCode = http.StatusInternalServerError
	}
	return
